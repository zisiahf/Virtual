(function(global) {
	var Timetables = function(option) {
		...
		this.gridOnClick = typeof option.gridOnClick === "function" ? option.gridOnClick
				: undefined;
		var styles = option.styles || {};
		this.leftHandWidth = styles.leftHandWidth || 40;
		this.Gheight = styles.Gheight || 48;
		this.defaultPalette = [ "#f05261", "#48a8e4", "#ffd061", "#52db9a",
				"#70d3e6", "#52db9a", "#3f51b5", "#f3d147", "#4adbc3",
				"#673ab7", "#f3db49", "#76bfcd", "#b495e1", "#ff9800",
				"#8bc34a" ];
		this.palette = (typeof styles.palette === "boolean" && !styles.palett) ? false
	...
	Timetables.prototype = {
		_init : function(option) {
			...
			var merge = typeof option.merge === "boolean" ? option.merge
					: this.merge;
			...
			if (typeof style.palette === "boolean" && !style.palette) {
				...
				palette = style.palette ? (style.palette || [])
						.concat(this.defaultPalette) : this.palette
			}
			var Timetables = option.timetables || this.Timetables;
			var week = option.week || this.week;
			var TimetableType = JSON.parse(JSON.stringify(option.timetableType
					|| this.TimetableType));
			var deepCopyTimetableType = option.timetableType
					|| this.TimetableType;
			var TimetableTypeLength = TimetableType.length;
			Timetables
					.forEach(function(item, index) {
						if (item.length < TimetableTypeLength) {
							for (var i = 0; i < TimetableTypeLength
									- item.length; i++) {
								item.push("")
							}
						}
						...
						.forEach(function(list, i) {
							...
							list
									.forEach(function(item, index) {
										...
										if (item === (listMerge[i][index - 1] || {}).name
												&& item) {
			...
			week
					.forEach(function(item, index) {
						...
						var highlightClass = highlightWeek === (index + 1) ? "highlight-week "
								: "";
						weekItem.className = highlightClass + "Courses-head-"
								+ (index + 1);
								...
								.forEach(function(item, i) {
									...
									courseItem.onclick = function(e) {
										...
										var info = {
											...
											week : week[i],
											index : index + 1,
											length : merge ? listMerge[i][index].length
													: 1
										};
			...
			leftHandTextDom.style.height ="29px";
